...
#     try:
#         df = pd.read_excel(file_path)
#         logging.info(f"Read {os.path.basename(file_path)} with columns: {df.columns.tolist()}")
#         return df
...
#                     draw_entry[time_of_day] = float(value)
#                 except ValueError:
#                     logging.warning(f"Could not convert value '{value}' to float for date {date} and time {time}. Skipping this entry.")
...
#     logging.info(f"Collection '{collection_name}' created with schema validation.")
#     return db[collection_name]
...
#                 upsert=True
#             )
#         logging.info(f"Uploaded {len(data)} documents to MongoDB")
#     else:
...
#     client = pymongo.MongoClient(mongodb_uri, connectTimeoutMS=60000, serverSelectionTimeoutMS=60000)
